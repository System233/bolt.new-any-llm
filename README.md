# 太拉了，别用
> [!CAUTION]
>* 用了几天，过程中patch-2分支修复了大量恶性bug，然而主线还在加垃圾功能根本没法合并。  
>* 内置的系统提示也太垃圾了，已经测试了llama8b，qwen2.5 coder 7B~32B、max\plus、GLM4 FLASH（GLM只有免费模型能够识别prompt，其他模型不管多大参数量全都不认提示），全部效果都非常差劲，v0dev的智能程度和代码完成能力在它们面前属于碾压级别的存在。
>* 实测运行效果和实用性不如与VSCODE集成性更高的cline，Cline只用了5分钟就让我明白了bolt是坨大便。
